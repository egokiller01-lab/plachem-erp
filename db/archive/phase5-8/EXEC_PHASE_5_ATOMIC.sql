...
-- SQL INTO BUG COMPLETELY RESOLVED BY ATOMIC ASSIGNMENTS
-- REQUIRED TABLE DEPENDENCIES INCLUDED
-- ==========================================

-- [0] 이전 단계(Phase 4) 누락으로 인한 필수 로그 테이블 강제 생성
...
BEGIN
    ...
    v_status := (SELECT status FROM public.production_headers WHERE id = p_doc_id);
    v_production_date := (SELECT production_date FROM public.production_headers WHERE id = p_doc_id);
    v_production_type := (SELECT production_type FROM public.production_headers WHERE id = p_doc_id);
    v_vendor_id := (SELECT vendor_id FROM public.production_headers WHERE id = p_doc_id);
    v_processing_fee := (SELECT processing_fee FROM public.production_headers WHERE id = p_doc_id);
    v_additional_cost := (SELECT additional_cost FROM public.production_headers WHERE id = p_doc_id);
    v_is_additional_cost_payable := (SELECT is_additional_cost_payable FROM public.production_headers WHERE id = p_doc_id);
    ...
    v_is_month_closed := (SELECT EXISTS (SELECT 1 FROM public.monthly_closings WHERE closing_year = to_char(v_production_date, 'YYYY') AND closing_month = to_char(v_production_date, 'MM') AND status = 'closed'));
    ...
    FOR v_item IN SELECT i.product_id, i.qty, p.moving_avg_cost, p.product_name FROM public.production_inputs i JOIN public.products p ON i.product_id = p.id WHERE i.production_header_id = p_doc_id LOOP
        v_current_stock := (SELECT stock_qty FROM public.v_product_stock WHERE product_id = v_item.product_id);
    ...
    v_total_output_qty := (SELECT SUM(qty) FROM public.production_outputs WHERE production_header_id = p_doc_id);
    IF COALESCE(v_total_output_qty, 0) > 0 THEN v_calculated_unit_cost := v_total_production_cost / v_total_output_qty; ELSE v_calculated_unit_cost := 0; END IF;
...
BEGIN
    ...
    v_ap_id := (SELECT id FROM public.accounts_payable WHERE ref_type = 'PRODUCTION_SUBCON' AND ref_id = p_doc_id LIMIT 1);
    v_ap_paid := (SELECT paid_amount FROM public.accounts_payable WHERE ref_type = 'PRODUCTION_SUBCON' AND ref_id = p_doc_id LIMIT 1);
...
BEGIN
    ...
    v_is_month_closed := (SELECT EXISTS (SELECT 1 FROM public.monthly_closings WHERE closing_year = to_char(p_date, 'YYYY') AND closing_month = to_char(p_date, 'MM') AND status = 'closed'));
    IF v_is_month_closed THEN RETURN jsonb_build_object('success', false, 'message', '해당 일자의 월 마감이 완료되어 지급을 등록할 수 없습니다.'); END IF;

    v_ap_status := (SELECT status FROM public.accounts_payable WHERE id = p_ap_id FOR UPDATE);
    v_ap_total := (SELECT total_amount FROM public.accounts_payable WHERE id = p_ap_id);
    v_ap_paid := (SELECT paid_amount FROM public.accounts_payable WHERE id = p_ap_id);
...
BEGIN
    ...
    v_status := (SELECT status FROM public.purchase_headers WHERE id = p_doc_id);
    v_purchase_date := (SELECT purchase_date FROM public.purchase_headers WHERE id = p_doc_id);
    v_supplier_id := (SELECT supplier_id FROM public.purchase_headers WHERE id = p_doc_id);
    v_due_date := (SELECT due_date FROM public.purchase_headers WHERE id = p_doc_id);
    v_purchase_no := (SELECT purchase_no FROM public.purchase_headers WHERE id = p_doc_id);

    IF v_status IS NULL THEN RETURN jsonb_build_object('success', false, 'message', '문서를 찾을 수 없습니다.'); END IF;
    IF v_status = 'confirmed' THEN RETURN jsonb_build_object('success', true, 'message', '이미 확정된 문서입니다.'); END IF;

    v_is_month_closed := (SELECT EXISTS (SELECT 1 FROM public.monthly_closings WHERE closing_year = to_char(v_purchase_date, 'YYYY') AND closing_month = to_char(v_purchase_date, 'MM') AND status = 'closed'));
    IF v_is_month_closed THEN RETURN jsonb_build_object('success', false, 'message', '해당 월이 마감되어 확정할 수 없습니다.'); END IF;

    v_total_amount := (SELECT SUM(net_amount + vat_amount) FROM public.purchase_items WHERE purchase_header_id = p_doc_id);
...
BEGIN
    ...
    v_ap_id := (SELECT id FROM public.accounts_payable WHERE ref_type = 'PURCHASE' AND ref_id = p_doc_id AND status != 'void' LIMIT 1);
    v_ap_paid := (SELECT paid_amount FROM public.accounts_payable WHERE ref_type = 'PURCHASE' AND ref_id = p_doc_id AND status != 'void' LIMIT 1);
...
BEGIN
    ...
    v_status := (SELECT status FROM public.sales_headers WHERE id = p_doc_id);
    v_sales_date := (SELECT sales_date FROM public.sales_headers WHERE id = p_doc_id);
    v_customer_id := (SELECT customer_id FROM public.sales_headers WHERE id = p_doc_id);
    v_due_date := (SELECT due_date FROM public.sales_headers WHERE id = p_doc_id);
    v_sales_no := (SELECT sales_no FROM public.sales_headers WHERE id = p_doc_id);

    IF v_status IS NULL THEN RETURN jsonb_build_object('success', false, 'message', '문서를 찾을 수 없습니다.'); END IF;
    IF v_status = 'confirmed' THEN RETURN jsonb_build_object('success', true, 'message', '이미 확정된 문서입니다.'); END IF;

    v_is_month_closed := (SELECT EXISTS (SELECT 1 FROM public.monthly_closings WHERE closing_year = to_char(v_sales_date, 'YYYY') AND closing_month = to_char(v_sales_date, 'MM') AND status = 'closed'));
    IF v_is_month_closed THEN RETURN jsonb_build_object('success', false, 'message', '해당 월이 마감되어 확정할 수 없습니다.'); END IF;

    v_total_amount := (SELECT total_amount FROM public.sales_headers WHERE id = p_doc_id);
...
BEGIN
    ...
    v_ar_id := (SELECT id FROM public.accounts_receivable WHERE ref_type = 'SALES' AND ref_id = p_doc_id AND status != 'void' LIMIT 1);
    v_ar_received := (SELECT received_amount FROM public.accounts_receivable WHERE ref_type = 'SALES' AND ref_id = p_doc_id AND status != 'void' LIMIT 1);
...
BEGIN
    ...
    v_ar_status := (SELECT status FROM public.accounts_receivable WHERE id = p_ar_id FOR UPDATE);
    v_ar_total := (SELECT total_amount FROM public.accounts_receivable WHERE id = p_ar_id);
    v_ar_received := (SELECT received_amount FROM public.accounts_receivable WHERE id = p_ar_id);
